...
int main()
{
    ...
    memset((char *)&serv_addr, 0, sizeof(serv_addr) + 1);
    ...
    */
    ...
    for (;;)
    {
        ...
        if ((n = select(sockfd + 1, &readset, NULL, NULL, NULL)) > 0)
        {
            ...
            {
                memset(&curMessage, 0, sizeof(message) + 1);
                ...
                if (curMessage.type == 'm' && curMessage.length > 0)
                {
                    curMessage.sender[strlen(curMessage.sender)];
                    printf("%s: %s", curMessage.sender, curMessage.value);
                }
                if (curMessage.type == 'n')
                {

                    curMessage.value[strlen(curMessage.value)];
        ...
        if (FD_ISSET(STDIN_FILENO, &readset))
        {
            ...
            memset(&messageToSend, 0, sizeof(message) + 1);
