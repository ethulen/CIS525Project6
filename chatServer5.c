				...
				if (FD_ISSET((people[i].sock), &readset))
				{
					...
					if (curmessage.type == 'n')
					{
						...
						if (namecheck == 0)
						{
							...
							if (i == 0 && curmessage.length > 0)
							{
								curmessage.type = 'm';
								strncpy(curmessage.sender, people[i].name, MAX);
								strncpy(curmessage.value, "You are the first to join the chat!\n", MAX);
								SSL_write(people[i].ssl, &curmessage, sizeof(message));
							}
							else{
								curmessage.type = 'm';
								strncpy(curmessage.sender, people[i].name, MAX);
								strncpy(curmessage.value, "welcome!\n", MAX);
								SSL_write(people[i].ssl, &curmessage, sizeof(message));
							}
